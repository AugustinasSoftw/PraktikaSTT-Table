...
import NavigationGmail from "./ComponentHelpers/Navigation/NavigationGmail";
...
export default function Share({ selectedFromIds, openShare, setOpenShare }: ShareProps) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[90] bg-black/0" onClick={() => setOpenShare(false)} />

      {/* popover container (top-right example; adjust position as you like) */}
      <div className="relative flex justify-end z-[90]" onClick={(e) => e.stopPropagation()}>
        <div
          role="dialog"
        ...
        >
          ...
          <div className="sticky top-0 z-10 flex items-center gap-2 h-14 px-4 border-b border-white/10 bg-zinc-800/80 backdrop-blur">
            ...
            {!openGmail ? (<div className="ml-auto flex items-center gap-1">
              <button
            ...
            </div>) : (<div className="ml-auto flex items-center gap-1">
              {openGmail && (
                <button
